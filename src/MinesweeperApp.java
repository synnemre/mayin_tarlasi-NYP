...
public class MinesweeperApp extends Application {
    ...
    // State encoding: 0=closed, 1=flagged, 2=opened-safe, 3=opened-mine,
    // 4=karga-highlight, 5=yilan-Y
    ...
    // Asset paths
    private static final String ASSET_YILAN = "assets/solucan.png"; // dosya adı değiştirilmeden, sadece referans ismi
                                                                    // düzeltildi
    ...
    private Image assetImgYukle(String yol, double boyut) {
        try {
            ...
            if (url == null)
                url = new java.io.File(yol).toURI().toURL();
    ...
    private void menuGoster() {
        ...
        leblebBtn.setOnAction(e -> {
            ...
            toplamLeblebPuani = 0;
            kaliciAltin = 0;
            toplamKargaKullanim = 0;
            toplamIlacKullanim = 0;
    ...
    private void klasikAyarDialogGoster() {
        ...
        int minMayinSayisi = (int) Math
                .floor(satirSpinner.getValueFactory().getValue() * sutunSpinner.getValueFactory().getValue() * 0.15);
        ...
        sep1.setStyle("-fx-background-color:" + ayrac + ";");
        Separator sep2 = new Separator();
        sep2.setStyle("-fx-background-color:" + ayrac + ";");
    ...
    // Property wrapper for fenerPulseOffset to use in Timeline
    ...
    private javafx.beans.property.DoubleProperty fenerPulseOffsetProperty() {
        if (fenerPulseOffsetProp == null) {
            fenerPulseOffsetProp = new javafx.beans.property.SimpleDoubleProperty(this, "fenerPulseOffset",
                    fenerPulseOffset) {
                @Override
                protected void invalidated() {
                    fenerPulseOffset = get();
                    geceKanvasiniCiz();
                }
            };
        }
    ...
    private void geceKanvasiniCiz() {
        ...
        boolean mouseOnBoard = fenerX >= 0 && fenerY >= 0
                && (leblebiBoardMode != null && !leblebiBoardMode.isOyunBitti());

        // Ambient darkness: when mouse is off-board use a lighter veil so the
        // grid is still barely visible; when the mouse is on-board a darker
        // veil is drawn first, then the lantern punches a hole through it.
        double ambientAlpha = mouseOnBoard ? 0.82 : 0.50;
        gc.setFill(javafx.scene.paint.Color.rgb(10, 10, 15, ambientAlpha));
        gc.fillRect(0, 0, w, h);

        if (mouseOnBoard) {
            double gercekYaricap = fenerBaseYaricap + fenerPulseOffset;
            // Lantern: radial gradient punches a softly-edged hole in the dark overlay.
            // Stop layout:
            // 0.00 → fully transparent (bright centre)
            // 0.70 → very faint dark tint (wide, comfortable lit zone)
            // 1.00 → full overlay alpha at the outer edge
            javafx.scene.paint.RadialGradient rg = new javafx.scene.paint.RadialGradient(
                    0, 0, fenerX, fenerY, gercekYaricap, false,
            ...
            gc.setFill(rg);
            gc.fillOval(fenerX - gercekYaricap, fenerY - gercekYaricap, gercekYaricap * 2, gercekYaricap * 2);
    ...
    private void marketPanelOlustur() {
        ...
        Button kargaBtn = marketKartButonOlustur("🐦",
                "Karga\n(" + (leblebiBoardMode != null ? leblebiBoardMode.getKargaLevel() : 1) + ". Seviye)", "20 💰");
        Button saatBtn = marketKartButonOlustur("⏰", "Emmi'nin Saati", "30 💰");
        Button ilacBtn = marketKartButonOlustur("🧪",
                "Zirai İlaç\n(" + (leblebiBoardMode != null ? leblebiBoardMode.getIlacLevel() : 1) + ". Seviye)",
                "50 💰");
    ...
    private static final String MKT_BTN_NORMAL = "-fx-background-color: #6b3e00; -fx-text-fill: #f5e6b0;" +
            "-fx-background-radius: 10; -fx-border-radius: 10;" +
            "-fx-border-color: #a07020; -fx-border-width: 1.5;" +
            "-fx-cursor: hand; -fx-alignment: center;";
    private static final String MKT_BTN_HOVER = "-fx-background-color: #8a5500; -fx-text-fill: #fff8e0;" +
            "-fx-background-radius: 10; -fx-border-radius: 10;" +
    ...
    private void marketPanelGuncelle() {
        if (marketPanel == null || leblebiBoardMode == null)
            return;
    ...
    private void gorevPanelOlustur() {
        ...
        gorevPanel.setStyle(
                "-fx-background-color: #2a1c02; -fx-border-color: #6b3e00; -fx-border-width: 0 2 0 0; -fx-min-width: 220px;");
    ...
    private void gorevPanelGuncelle() {
        if (gorevPanel == null || leblebiBoardMode == null)
            return;
        ...
        for (LeblebiBoardMode.Gorev g : leblebiBoardMode.getAktifGorevler()) {
            ...
            desc.setStyle("-fx-font-size: 14px; -fx-text-fill: " + (g.tamamlandi ? "#a6e3a1" : "#e8c55a")
                    + "; -fx-font-weight: bold;");
    ...
    private void hucreAc(int s, int u) {
        if (leblebModu && leblebiBoardMode != null) {
            ...
            if (leblebiBoardMode.isZirayiIlacAktif()) {
                // BUG-1 FIX: LBM.hucreAc() üzerinden geç — yokEdilenYilan sayıcısı orada
                // güncelleniyor
    ...
    private void isaretKoy(int s, int u) {
        ...
        if (!isaretliydi && yerlestirilenIsaret >= mayinSayisi)
            return;
    ...
    private Board aktifTahta() {
        if (leblebModu && leblebiBoardMode != null)
            return leblebiBoardMode.getTahta();
        if (klasikBoardMode != null)
            return klasikBoardMode.getTahta();
        return null;
    }
    ...
    private void arayuzuGuncelle() {
        ...
        java.util.List<int[]> kargaKonumlar = (leblebModu && leblebiBoardMode != null)
                ? leblebiBoardMode.getKargaGosterilenMayinlar()
                : null;
        ...
        for (int s = 0; s < satirSayisi; s++) {
            for (int u = 0; u < sutunSayisi; u++) {
                ...
                if (yilanY)
                    nyDurum = 5;
                else if (hucre.isAcildiMi() && hucre.isMayinMi())
                    nyDurum = 3;
                else if (hucre.isAcildiMi() && hucre.isGoldenLeblebi())
                    nyDurum = 20;
                else if (hucre.isAcildiMi())
                    nyDurum = (byte) (10 + hucre.getKomsuMayinSayisi());
    ...
    private void altinLeblebiAnimasyonu(Button btn) {
        ...
        artiOn.setStyle(
                "-fx-font-size: 20px; -fx-font-weight: bold; -fx-text-fill: #ffd700; -fx-effect: dropshadow(gaussian, rgba(0,0,0,0.5), 4, 0, 0, 2);");
    ...
    private void seviyeGecisiniGoster() {
        ...
        Label altBaslik = new Label(
                sonSeviye ? "Mehmet Emmi çok mutlu!" : "Seviye " + mevcutSeviye + " Başarıyla Bitti!");
        ...
        if (rapor.altinLeblebiBulundu() > 0) {
            detaylar.getChildren().add(satirOlustur.apply("Altın Leblebi (" + rapor.altinLeblebiBulundu() + "):",
                    "+" + (rapor.altinLeblebiBulundu() * 10)));
        }
        ...
        toplamLabel
                .setStyle("-fx-font-size: 18px; -fx-font-weight: bold; -fx-text-fill: #d84315; -fx-padding: 10 0 0 0;");

        Label emmiYorumu = new Label("💬 " + rapor.emmiYorumu());
        emmiYorumu.setWrapText(true);
        emmiYorumu.setStyle(
                "-fx-font-size: 14px; -fx-font-style: italic; -fx-text-fill: #5d4037; -fx-background-color: #d7ccc8; -fx-padding: 10; -fx-background-radius: 5;");
        ...
        btnSonraki.setStyle(
                "-fx-background-color: #4caf50; -fx-text-fill: white; -fx-font-size: 16px; -fx-font-weight: bold; -fx-padding: 8 20; -fx-cursor: hand; -fx-background-radius: 5;");
        btnSonraki.setOnAction(e -> {
            if (anaSahneKoku != null)
                anaSahneKoku.getChildren().remove(overlay);
            if (sonSeviye) {
                oyunSonuPopupuGoster();
            } else {
                // Snapshot gold and item-upgrade counters so they carry into the next level
                if (leblebiBoardMode != null) {
                    kaliciAltin = leblebiBoardMode.getAltin();
                    toplamKargaKullanim = leblebiBoardMode.getKargaKullanimToplami();
                    toplamIlacKullanim = leblebiBoardMode.getIlacKullanimToplami();
                }
                mevcutSeviye++;
                leblebOyunuBaslat();
            }
        ...
        btnMenu.setStyle(
                "-fx-background-color: #e0e0e0; -fx-text-fill: #424242; -fx-font-size: 14px; -fx-padding: 8 15; -fx-cursor: hand; -fx-background-radius: 5;");
    ...
    private void klasikZamanliSkorKaydet() {
        if (klasikBoardMode == null || klasikBoardMode.isSkorKaydedildi())
            return;
    ...
    private javafx.scene.Node scrollSar(javafx.scene.Node icerik, String arka) {
        ...
        scroll.viewportBoundsProperty().addListener((obs, ov, nv) -> {
            ...
            if (vp != null)
                vp.setStyle("-fx-background-color:" + arka + ";");
        });
            ...
            bar.getChildren().forEach(n -> {
                if (n instanceof Label l)
                    ...
                    b.setStyle(b.getStyle() +
                            "-fx-background-color:"
                            + (karanlikTema && !leblebModu ? "#313244" : (leblebModu ? "#7a5200" : "#c8cdd8")) + ";" +
    ...
    private String acilmisHucreTarzi(int k, String[] sr) {
        return acilmisHucreTarzi(k, sr, 14);
    }
    ...
    private void globalCssUygula(Scene hedefSahne) {
        ...
        try {
            ...
            if (fontUrl == null)
                fontUrl = new java.io.File(FONT_YOL).toURI().toURL();
        ...
        String css = ".root { -fx-font-family: '" + fontFamily + "'; }" +
    ...
    private void ekranSarsintisi() {
        ...
        aktifSarsinti = new Timeline(
                ...
                new KeyFrame(Duration.millis(40),
                        new javafx.animation.KeyValue(merkezIcerikKutusu.scaleXProperty(), 0.97)),
                new KeyFrame(Duration.millis(80),
                        new javafx.animation.KeyValue(merkezIcerikKutusu.scaleXProperty(), 1.03)),
                new KeyFrame(Duration.millis(120),
                        new javafx.animation.KeyValue(merkezIcerikKutusu.scaleXProperty(), 0.97)),
                new KeyFrame(Duration.millis(160),
                        new javafx.animation.KeyValue(merkezIcerikKutusu.scaleXProperty(), 1.03)),
                new KeyFrame(Duration.millis(200),
                        new javafx.animation.KeyValue(merkezIcerikKutusu.scaleXProperty(), 0.98)),
                new KeyFrame(Duration.millis(240),
                        new javafx.animation.KeyValue(merkezIcerikKutusu.scaleXProperty(), 1.02)),
                new KeyFrame(Duration.millis(280),
                        new javafx.animation.KeyValue(merkezIcerikKutusu.scaleXProperty(), 1.0)));
    ...
    private void canIkonunuKir(int kalanCan) {
        ...
        if (kirIndex < 0 || kirIndex >= canIkonKutusu.getChildren().size())
            return;
        ...
        buyuKucul.setFromX(1.0);
        buyuKucul.setToX(1.4);
        buyuKucul.setFromY(1.0);
        buyuKucul.setToY(1.4);
    ...
    private void canSayisiGuncelle(int canSayisi) {
        ...
        if (kokDuzen != null && kokDuzen.getTop() instanceof HBox bar) {
            if (!bar.getChildren().contains(canIkonKutusu)) {
                ...
                if (idx >= 0)
                    bar.getChildren().add(idx + 1, canIkonKutusu);
                else
                    bar.getChildren().add(canIkonKutusu);
            }
        }
    }
    ...
    private void diyalogGoster(String metin) {
        if (konusmaBalonuPanel == null || konusmaBalonuLabel == null)
            return;
        ...
        kaybol.setOnFinished(e -> {
            konusmaBalonuPanel.setVisible(false);
            if (leblebiBoardMode != null)
                leblebiBoardMode.diyaloguTemizle();
            aktifBalonAnimasyonu = null;
        });
    ...
    /**
     * 3x3 alanin tum hucrelerine ilac highlight stili uygular + cursor crosshair
     * yapar.
     */
    private void ilacHoverUygula(int merkezS, int merkezU) {
        if (dugmeler == null || aktifTahta() == null || leblebiBoardMode == null)
            return;
        Board tahta = aktifTahta();
        ...
        if (cap == 100) {
            for (int s = 0; s < satirSayisi; s++)
                etkiAlani.add(new int[] { s, merkezU });
            for (int u = 0; u < sutunSayisi; u++) {
                if (u != merkezU)
                    etkiAlani.add(new int[] { merkezS, u });
            }
        } else {
        ...
        for (int[] pos : etkiAlani) {
            ...
            if (ys < 0 || ys >= satirSayisi || yu < 0 || yu >= sutunSayisi)
                continue;
            Button hedef = dugmeler[ys][yu];
            if (hedef == null || hedef.isDisabled())
                continue;
    ...
    /**
     * Tum grid hucrelerinin ilac highlight'larini temizler, cursor'u normale
     * dondurur.
     */
    private void ilacHoverTemizle() {
        if (dugmeler == null || aktifTahta() == null)
            return;
        Board tahta = aktifTahta();
    ...
    /**
     * Merkez h\u00fccre: parlak ye\u015fil-sar\u0131 highlight, kal\u0131n
     * kenarlık.
     */
